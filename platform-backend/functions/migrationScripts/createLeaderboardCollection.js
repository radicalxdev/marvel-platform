...
const createLeaderboardCollection = https.onCall(async (data, context) => {
  const challengesDocs = await admin.firestore().collection("challenges")
    .where("status", "in", [STATUS.IN_PROGRESS, STATUS.COMPLETED]).get();

  const challengePromises = challengesDocs.docs.map(async (docRef) => {
    const challengeId = docRef.id;
    const enrolledPlayerDocs = await admin.firestore().collection("enrolledPlayers")
      .where("challengeId", "==", challengeId).get();

    let players = [];
    await Promise.all(enrolledPlayerDocs.docs.map(async (doc) => {
      const enrolledPlayerDoc = doc.data();
      const answersDoc = await doc.ref.collection("internal").doc("tasksAnswers").get();
      if (!answersDoc.exists) return;

      const answersData = answersDoc.data();
      const currentTime = new Date();
      if (moment(currentTime).diff(answersData.startTime?.toDate(), "milliseconds") <
        TWENTY_FIVE_MINS &&
      ...
      let userDoc = await admin.firestore().collection("users")
        .doc(enrolledPlayerDoc.userId).get();
