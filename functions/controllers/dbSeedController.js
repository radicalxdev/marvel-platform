...
const seedDatabase = functions.https.onCall(async (data, context) => {
  ...
  if (markerDoc.exists) {
    ...
    try {
      ...
      for (const key in seedData) {   
        const doc = seedData[key];
        const docRef = db.collection("tools").doc(doc.id.toString());
        batch.set(docRef, doc);
      }
      await batch.commit();
      await db.collection('markers').doc('seedingDone').set({ timestamp: new Date() });
      ...
      throw new functions.https.HttpsError('unknown', 'Failed to seed database', error);
    }
  }
});
