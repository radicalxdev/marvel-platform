...
exports.signUpUser = https.onCall(async (data, context) => {
  ...
  // Input validations
  if (!email) {
    throw new https.HttpsError('invalid-argument', 'Email is required.', { code: 400 });
  }
  if (!isValidEmail(email)) {
    throw new https.HttpsError('invalid-argument', 'Invalid email format.', { code: 400 });
  }
  if (!uid || typeof uid !== 'string' || uid.trim() === '') {
    throw new https.HttpsError('invalid-argument', 'UID is required and must be a non-empty string.', { code: 400 });
  }
  if (!fullName || typeof fullName !== 'string' || fullName.trim().length === 0 || fullName.length > 100) {
    throw new https.HttpsError('invalid-argument', 'Full name is required, must be a string, and should be between 1 and 100 characters.', { code: 400 });
  }
  ...
  try {
    ...
    return { status: 'success', message: 'User document created successfully', user: userDoc };
  } catch (error) {
    ...
    throw new https.HttpsError('internal', 'Firebase internal error.', { code: 500 });
  }
});
